...
describe("dimension-scorer — scoring functions", () => {
  it("scoreApplication returns 0 for empty input", () => {
    assert.equal(scoreApplication([]), 0);
  });

  it("scoreApplication scores based on agent breadth and messages", () => {
    const summaries = [
  ...
  it("scoreApplication excludes platform agents", () => {
    ...
    assert.equal(scoreApplication(summaries), 0);
  });
  ...
  it("scoreOrchestration returns 0 for no agents", () => {
    assert.equal(scoreOrchestration([]), 0);
  });

  it("scoreOrchestration rewards goals and cron", () => {
    const agents = [
      { id: "coder", name: "Coder", goals: [{ id: "g1", enabled: true }], cron: [{ schedule: "0 9 * * *" }] },
      { id: "writer", name: "Writer" },
      { id: "researcher", name: "Researcher", mcps: ["github"] },
    ];
    const score = scoreOrchestration(agents as any);
    assert.ok(score >= 2, `Expected score >= 2, got ${score}`);
  });

  it("scoreCraft returns 0 for no agents", () => {
    assert.equal(scoreCraft([]), 0);
  });

  it("scoreCraft rewards custom agents with good prompts", () => {
    const agents = [
    ...
    const score = scoreCraft(agents as any);
  ...
  it("scoreAllDimensions returns all 5 keys", () => {
    ...
    assert.ok("orchestration" in result);
    assert.ok("craft" in result);
  });

  it("computeTrends detects improvements", () => {
    const current = { application: 3, communication: 2, knowledge: 3, orchestration: 1, craft: 2 };
    const previous = { application: 2, communication: 2, knowledge: 2, orchestration: 1, craft: 1 };
    const trends = computeTrends(current, previous);
    assert.equal(trends.application, "up");
    assert.equal(trends.communication, "stable");
    assert.equal(trends.knowledge, "up");
    assert.equal(trends.orchestration, "stable");
    assert.equal(trends.craft, "up");
  });

  it("computeTrends returns stable for null previous", () => {
    const current = { application: 3, communication: 2, knowledge: 3, orchestration: 1, craft: 2 };
    const trends = computeTrends(current, null);
    assert.equal(trends.application, "stable");
    assert.equal(trends.communication, "stable");
  });
  ...
  it("all scores are clamped 0-5", () => {
    ...
    const score = scoreApplication(bigSummaries);
