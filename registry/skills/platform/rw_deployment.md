...
**Important:** "Remove deployment" (`railway down`) stops the current deployment but keeps the service. To delete a service entirely, use the `rw_environment` skill with `isDeleted: true`.
...
Useful when:
- Config changed via rw_environment skill
...
**Note:** This does NOT delete the service. To delete a service entirely, use the `rw_environment` skill with `isDeleted: true`.
...
- **Push new code**: Use `rw_deploy` skill
- **Check service status**: Use `rw_status` skill
- **Fix config issues**: Use `rw_environment` skill
- **Create new service**: Use `rw_new` skill
