...
- User wants to deploy from a GitHub repo (create empty service first via `rw_new` skill, then use this)
...
Use `merge: true` to automatically merge with existing staged changes.
...
- **Create service**: Use `rw_service` skill
- **View logs**: Use `rw_deployment` skill
- **Add domains**: Use `rw_domain` skill
