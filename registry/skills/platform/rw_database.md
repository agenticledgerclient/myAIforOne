...
For non-database templates, see the `rw_templates` skill.
...
User mentions database
        |
Check existing DBs first (query env config for source.image)
        |
   +----+----+
...
Use `rw_environment` skill to add the variable reference:
...
6. Use `rw_environment` skill to stage: `DATABASE_URL: { "value": "${{Postgres.DATABASE_URL}}" }`
7. Apply changes
...
1. Check existing DBs via env config query
2. If redis exists: Wire up REDIS_URL via rw_environment skill -> apply
...
- **Connect services**: Use `rw_environment` skill to add variable references
- **View database service**: Use `rw_service` skill
- **Check logs**: Use `rw_deployment` skill
