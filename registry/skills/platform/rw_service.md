...
**Note:** For creating services with local code (the common case), prefer the `rw_new` skill which handles project setup, scaffolding, and service creation together.

**For GitHub repo sources:** Use `rw_new` skill to create empty service, then `rw_environment` skill to configure source.repo via staged changes API.
...
2. Use `rw_environment` skill to configure source via staged changes API
3. Apply to trigger deployment

### After Creating: Configure Instance

Use `rw_environment` skill to configure the service instance:
...
Then use `rw_environment` skill to apply and deploy.
...
- **Create service with local code**: Use `rw_new` skill (handles scaffolding + creation)
- **Configure service**: Use `rw_environment` skill (variables, commands, image, etc.)
- **Delete service**: Use `rw_environment` skill with `isDeleted: true`
- **Apply changes**: Use `rw_environment` skill
- **View logs**: Use `rw_deployment` skill
- **Deploy local code**: Use `rw_deploy` skill
