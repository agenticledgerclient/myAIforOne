...
For subdirectory deployments, prefer setting `rootDirectory` via the rw_environment skill, then deploy normally with `railway up`.
...
```
Use `rw_deployment` skill to check build status (with `--lines` flag).

### CI mode
Build logs stream inline. If build fails, the error will be in the output.

**Do NOT run `railway logs --build` after CI mode** - the logs already streamed. If you need more context, use `rw_deployment` skill with `--lines` flag (never stream).

## Composability

- **Check status after deploy**: Use `rw_service` skill
- **View logs**: Use `rw_deployment` skill
- **Fix config issues**: Use `rw_environment` skill
- **Redeploy after config fix**: Use `rw_environment` skill
...
- Build command wrong - use rw_environment skill to fix
