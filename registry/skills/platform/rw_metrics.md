...
- User asks "is my service healthy?" (combine with `rw_service` skill)
...
- **Get IDs**: Use `rw_status` skill or `railway status --json`
- **Check service health**: Use `rw_service` skill for deployment status
- **View logs**: Use `rw_deployment` skill if metrics show issues
- **Scale service**: Use `rw_environment` skill to adjust resources

## Error Handling

### Empty/Null Metrics

Services without active deployments return empty metrics arrays.

When processing with jq, handle nulls:
